package ru.sis.cosplay.jphash.image;
...
public class SimpleGrayscaleImage {
  ...
  public void blur() {
    ...
    for (int y = 0; y < height; y++) {
      ...
      t =
          ((buffer_1[0] + buffer[1] + buffer_1[2] + buffer_2[3]) * 455)
              >> 10; // 1024/455 = 2.250549
            ...
            (int)
                    (((long)
                            (buffer_2[x - 2]
                                ...
                                + buffer_2[x + 2]))
                        * 409)
                >> 10; // 1024/409 = 2.503 ~ 1 + 2*0.5 + 2*0.25
        this.data.put(width * y + x, (byte) (t > 255 ? 255 : t));
      }
      int x = width - 2;
      t =
          ((buffer_2[x - 2] + buffer_1[x - 1] + buffer[x] + buffer_1[x + 1]) * 455)
              >> 10; // 1024/455 = 2.250549
      this.data.put(width * y + x, (byte) (t > 255 ? 255 : t));
      x++;
    ...
    for (int x = 0; x < width; x++) {
      ...
      // y = 0
      this.data.put(
          x, (byte) (((buffer[0] + buffer_1[1] + buffer_2[2]) * 585) >> 10)); // 1024/585 = 1.75042
      // y = 1
      this.data.put(
          width + x,
          (byte)
      ...
      for (int y = 2; y < height - 2; y++) {
        long t =
            (((long)
                ...
                >> 10;
        this.data.put(
            width * y + x, (byte) (t > 255 ? 255 : t)); // 1024/409 = 2.503 ~ 1 + 2*0.5 + 2*0.25
      }
      int y = height - 2;
      this.data.put(
          width * y + x,
          (byte)
              (((buffer_2[y - 2] + buffer_1[y - 1] + buffer[y] + buffer_1[y + 1]) * 455)
                  >> 10)); // 1024/455 = 2.250549
      y++;
      this.data.put(
          width * y + x,
          (byte)
              (((buffer_2[y - 2] + buffer_1[y - 1] + buffer[y]) * 585)
                  >> 10)); // 1024/585 = 1.75042
    }
  }
  ...
  public void loadImage(BufferedImage image) {
    ...
    if (image.getColorModel().getComponentSize(0) == BYTE_SIZE) {
      ...
      try {
        ...
        System.out.println(
            "Died trying to allocate a buffer of size: "
                + bufferSize
      ...
      if (numComponents == 1) {
        ...
        for (int i = 0; i < bufferSize; i += numComponents) {
          // V2: CImg version
          int y =
              (((tempBuffer[i + 2] & 0xFF) * 66
                          + (tempBuffer[i + 1] & 0xFF) * 129
                          + (tempBuffer[i] & 0xFF) * 25)
                      >> 8)
                  + 16;
        ...
        throw new IllegalArgumentException(
            "Can't work with this type of byte image: " + image.getType());
      }
    } else {
