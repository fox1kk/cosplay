...
import ru.sis.cosplay.jphash.image.SimpleGrayscaleImage;
...
public class RadialHashAlgorithm {
  ...
  public static RadialHash getHash(BufferedImage img) {
  ...
  private static RadialHash calculateHash(Features features) {
    ...
    for (int k = 0; k < nb_coeffs; k++) {
      ...
      if (k == 0) D_temp[k] = sum / Math.sqrt((double) N);
      else D_temp[k] = sum * SQRT_TWO / Math.sqrt((double) N);
